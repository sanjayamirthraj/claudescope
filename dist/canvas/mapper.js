...
export class CourseMapper {
    courseMappings = [];
    assignmentMappings = new Map();
    // Auto-match Canvas courses to Gradescope courses
    autoMatchCourses(canvasCourses, gradescopeCourses) {
        const matched = [];
        const unmatched = [];
        const usedGradescopeIds = new Set();
        for (const canvasCourse of canvasCourses) {
            let bestMatch = null;
            let bestScore = 0;
            for (const gseCourse of gradescopeCourses) {
                if (usedGradescopeIds.has(gseCourse.id))
                    continue;
                // Compare course name and code
                const nameScore = stringSimilarity(canvasCourse.name, gseCourse.name);
                const codeScore = stringSimilarity(canvasCourse.course_code, gseCourse.shortName);
    ...
    manualMapCourse(canvasCourse, gradescopeCourse) {
        // Remove any existing mapping for this Canvas course
        this.courseMappings = this.courseMappings.filter((m) => m.canvasCourseId !== canvasCourse.id);
        const mapping = {
            canvasCourseId: canvasCourse.id,
            canvasCourseName: canvasCourse.name,
    ...
    excludeCourse(canvasCourseId) {
        const mapping = this.courseMappings.find((m) => m.canvasCourseId === canvasCourseId);
        if (mapping) {
    ...
    autoMatchAssignments(canvasCourseId, canvasAssignments, gradescopeAssignments) {
        ...
        for (const canvasAssignment of canvasAssignments) {
            ...
            for (const gsAssignment of gradescopeAssignments) {
                if (usedGradescopeIds.has(gsAssignment.id))
                    continue;
                const score = stringSimilarity(canvasAssignment.name, gsAssignment.name);
